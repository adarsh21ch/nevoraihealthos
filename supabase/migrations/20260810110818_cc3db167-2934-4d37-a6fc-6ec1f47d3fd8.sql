-- Restore public search_path to the authenticator role.
...
-- Restore core schema usage for ALL internal proxy roles
...
-- Restore USAGE on the specific role type (app_role) to internal roles
GRANT USAGE ON TYPE public.app_role TO authenticator, supabase_auth_admin, anon, authenticated, service_role;

-- Audit and Ensure Permissions on user_roles and profiles
...
-- Ensure execute on critical security functions
GRANT EXECUTE ON FUNCTION public.is_platform_admin(uuid) TO authenticated, supabase_auth_admin;
GRANT EXECUTE ON FUNCTION public.has_role(uuid, public.app_role) TO authenticated, supabase_auth_admin;

-- Final check: Ensure the admin user role is explicitly set
