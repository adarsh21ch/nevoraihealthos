...
import { Utensils, ChefHat, CheckCircle2, RefreshCw, MessageSquare, Sparkles, Target, ArrowRight, ChevronRight, AlertCircle, Activity, Flame, Clock } from "lucide-react";
...
export function PersonalizedPlan() {
    ...
    const { data: mealLogs } = useQuery({
        ...
        queryFn: () => getLogs({ data: { date: today } }),
        enabled: !!plan
    });
    ...
    if (isReadinessLoading || (readiness?.ready && isPlanLoading)) {
        return (<div className="space-y-10 py-4">
    ...
    if (readiness && !readiness.ready) {
        return (<div className="bg-white border border-slate-100 rounded-[2.5rem] p-10 text-center space-y-8 shadow-sm animate-in fade-in duration-700">
        ...
        <div className="bg-slate-50 rounded-3xl p-6 space-y-4 text-left">
           ...
           <div className="space-y-2 pt-2">
             ...
             <div className="grid grid-cols-1 gap-2">
               {readiness.missing.slice(0, 4).map((m) => (<div key={m.field} className="flex items-center gap-2 text-slate-600">
                    <AlertCircle className="w-3 h-3 text-amber-400"/>
                    <span className="text-xs font-bold">{m.label}</span>
                 </div>))}
               {readiness.missing.length > 4 && (<span className="text-[9px] font-bold text-slate-400 italic">+{readiness.missing.length - 4} more details</span>)}
             </div>
           </div>
        </div>

        <Button asChild className="w-full h-16 rounded-2xl bg-health-green hover:bg-health-green/90 text-white font-bold text-sm shadow-xl shadow-emerald-100">
    ...
    if (!plan && !generateMutation.isPending) {
        ...
        <div className="flex flex-col gap-4">
          <Button onClick={() => generateMutation.mutate()} className="w-full h-16 rounded-2xl bg-health-green hover:bg-health-green/90 text-white font-bold text-sm shadow-xl shadow-emerald-100">
            Create My Plan
          </Button>
          <Button variant="ghost" asChild className="text-[10px] font-black uppercase tracking-widest text-slate-400">
    ...
    if (generateMutation.isPending) {
        return (<div className="bg-white border border-slate-100 rounded-[2.5rem] p-12 text-center space-y-10 shadow-sm animate-in fade-in duration-700">
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="flex gap-1.5">
            {[...Array(3)].map((_, i) => (<div key={i} className={cn("w-2 h-2 rounded-full bg-health-green animate-bounce", i === 1 && "delay-100", i === 2 && "delay-200")}/>))}
          </div>
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Personalizing Indian meal options...</span>
        </div>
      </div>);
    }
    const planData = plan ? plan.plan_data : null;
    const customer = profile;
    if (!planData)
        return null;
    // 19. MY PLAN — GENERATED STATE
    return (<div className="space-y-10 animate-in fade-in duration-1000">
      ...
      <section className="bg-emerald-900 text-white rounded-[2.5rem] p-8 space-y-8 shadow-xl shadow-emerald-900/10 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            {customer?.diet_preference && (<span className="px-3 py-1.5 bg-white/5 rounded-xl text-[9px] font-bold text-white/70 border border-white/10">{customer.diet_preference}</span>)}
            {customer?.cooking_access && (<span className="px-3 py-1.5 bg-white/5 rounded-xl text-[9px] font-bold text-white/70 border border-white/10">{customer.cooking_access}</span>)}
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
           ...
           <button onClick={() => generateMutation.mutate()} className="text-[10px] font-black uppercase tracking-widest text-health-green flex items-center gap-1.5 opacity-60 hover:opacity-100 transition-opacity">
        ...
        <div className="space-y-6">
          {planData.meals?.map((meal, idx) => {
            ...
            return (<div key={meal.id} className="relative group">
                {idx !== planData.meals.length - 1 && (<div className="absolute left-[29px] top-16 bottom-0 w-px bg-slate-100 group-hover:bg-health-green/20 transition-colors"/>)}
                
                <div className={cn("bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm transition-all duration-500 relative z-10", isCompleted ? "opacity-60 border-emerald-100 bg-emerald-50/20" : "hover:border-health-green/20 hover:shadow-md")}>
                  <div className="flex justify-between items-start mb-5">
                    <div className="flex items-center gap-4">
                      <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500", isCompleted ? "bg-health-green text-white shadow-lg shadow-emerald-900/10" : "bg-slate-50 text-slate-300 group-hover:text-health-green")}>
                  ...
                  <div className="space-y-4 pl-1">
                    <div className="flex flex-wrap gap-2">
                      {meal.foods?.map((food) => (<span key={food} className="px-3 py-1.5 rounded-xl bg-slate-50 text-[10px] font-bold text-slate-600 border border-slate-100 group-hover:bg-white transition-colors">
                          {food}
                        </span>))}
                    </div>
                    
                    {meal.protein_content && (<div className="flex items-center gap-1.5">
                    ...
                    <div className="flex gap-3 pt-3">
                      <Button variant="outline" size="sm" disabled={isCompleted || logMutation.isPending} onClick={() => logMutation.mutate({ mealId: meal.id, status: 'COMPLETED' })} className={cn("h-12 flex-1 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all", isCompleted ? "bg-health-green border-health-green text-white" : "border-slate-100 text-slate-500 hover:border-health-green hover:text-health-green")}>
                        {isCompleted ? "Completed" : "Mark as Done"}
                      </Button>
                      <Button variant="ghost" size="sm" className="h-12 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-ink transition-colors">
                        Substitutions
                      </Button>
                    </div>
                  </div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 space-y-6">
        ...
        <div className="relative">
          <input type="text" placeholder="Ask your Fat2Fit Coach AI..." className="w-full h-16 pl-6 pr-16 rounded-2xl bg-slate-50 border-none focus:ring-2 focus:ring-health-green/10 text-sm font-medium"/>
