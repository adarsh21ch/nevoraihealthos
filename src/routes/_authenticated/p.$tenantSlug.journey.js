...
import { CheckCircle2, Lock, Trophy, Sparkles, ChevronRight, AlertCircle } from 'lucide-react';
...
function JourneyPage() {
    ...
    if (isLoading) {
        return (<div className="max-w-md mx-auto px-6 pt-16 pb-32 space-y-12">
        ...
        <div className="space-y-6">
          {[...Array(5)].map((_, i) => (<div key={i} className="flex gap-6 items-start">
    ...
    if (!data || 'state' in data && data.state === 'no_content') {
        return (<div className="min-h-[60vh] flex flex-col items-center justify-center p-8 text-center space-y-8">
    ...
    const { customer, program, programDays } = data;
    const duration = program?.duration_days || 9;
    const currentDay = customer?.start_date ? getProgramDayNumber(customer.start_date) : 1;
    return (<div className="max-w-md mx-auto px-6 pt-16 pb-32 animate-in fade-in duration-1000">
      {/* Editorial Header */}
      <header className="mb-12 relative">
        ...
        <div className="flex items-baseline gap-3 mt-6">
          ...
          <div className="h-1 flex-1 bg-slate-100 rounded-full relative overflow-hidden">
            <div className="absolute inset-y-0 left-0 bg-health-green transition-all duration-1000" style={{ width: `${Math.min(100, (currentDay / duration) * 100)}%` }}/>
          </div>
        </div>
      </header>

      {/* AI Coach Milestone Insight */}
      {insights?.message && (<section className="bg-emerald-900 text-white rounded-[2.5rem] p-8 mb-12 relative overflow-hidden shadow-2xl shadow-emerald-900/20">
      ...
      <div className="relative space-y-6">
        ...
        {[...Array(duration)].map((_, i) => {
            ...
            return (<div key={dayNum} className={cn("flex gap-6 items-start group transition-all duration-500", isPast && "opacity-50")}>
              {/* Day Indicator Node */}
              <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border-2 z-10 transition-all duration-500 mt-2", isToday ? "bg-white border-health-green scale-110 shadow-2xl shadow-health-green/20" :
                ...
                {isPast ? (<CheckCircle2 className="w-7 h-7"/>) : isFuture ? (<Lock className="w-5 h-5 opacity-40"/>) : (<span className="font-bold text-2xl italic font-serif text-ink">{dayNum}</span>)}
              </div>

              {/* Day Content Card */}
              <Link to={isFuture ? "/p/$tenantSlug/journey" : "/p/$tenantSlug/today"} params={{ tenantSlug }} disabled={isFuture} className={cn("flex-1 p-6 rounded-[2.2rem] border transition-all duration-500 text-left relative overflow-hidden", isToday ? "bg-white border-slate-100 shadow-md ring-1 ring-slate-100/50" :
                    isPast ? "bg-slate-50/50 border-transparent" :
                        "bg-transparent border-transparent")}>
                <div className="flex justify-between items-center">
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <h3 className={cn("font-bold text-xl tracking-tight leading-none", isFuture ? "text-slate-400" : "text-ink")}>
                        Day {dayNum}
                      </h3>
                      {isToday && (<span className="px-2 py-0.5 bg-health-green/10 text-health-green text-[9px] font-black uppercase tracking-widest rounded-full">Active</span>)}
                      {isPast && (<span className="text-[9px] font-black text-health-green uppercase tracking-widest">Completed</span>)}
                    </div>
                    {dayInfo?.title ? (<span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">{dayInfo.title}</span>) : (<span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">Protocol Locked</span>)}
                  </div>
                  {!isFuture && (<ChevronRight className={cn("w-5 h-5 transition-transform duration-300 group-hover:translate-x-1", isToday ? "text-health-green" : "text-slate-300")}/>)}
                </div>
                
                {isToday && (<div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-1 duration-700">
                    <p className="text-sm text-slate-500 font-medium leading-relaxed line-clamp-2">
                      {dayInfo?.focus || "Focusing on your metabolic reset through hydration and movement."}
                    </p>
                    <Button asChild className="w-full bg-health-green text-white rounded-2xl font-bold h-12 shadow-lg shadow-emerald-100">
                ...
                {isFuture && (<div className="mt-2 flex items-center gap-2">
