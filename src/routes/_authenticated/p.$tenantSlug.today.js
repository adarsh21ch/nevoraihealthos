...
import { Droplets, Info, Calendar, Smile, Frown, Meh, Sparkles, ArrowRight, Activity, Zap, Check } from 'lucide-react';
...
function TodayPage() {
    ...
    if (isLoading) {
        return (<div className="max-w-md mx-auto px-6 pt-12 pb-24 space-y-8">
    ...
    if (error) {
        return (<div className="min-h-[60vh] flex flex-col items-center justify-center p-8 text-center space-y-6">
    ...
    if (state !== 'success') {
        ...
        {state === 'not_a_customer' && (<Button onClick={() => navigate({ to: '/onboarding' })} className="bg-health-green rounded-2xl text-white">
            Start Onboarding
          </Button>)}
    ...
    const { customer, dayContent, dailyLog, dayNumber } = data;
    ...
    const primaryTask = dayContent?.tasks?.find((t) => !dailyLog?.task_completions?.some((c) => c.day_task_id === t.id)) || dayContent?.tasks?.[0];
    ...
    const tasksBySlot = dayContent?.tasks?.reduce((acc, task) => {
    ...
    return (<div className="max-w-md mx-auto px-6 pt-12 pb-32 animate-in fade-in duration-500 space-y-8">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="relative w-20 h-20 flex items-center justify-center">
          <svg className="w-full h-full -rotate-90">
            ...
            <circle cx="40" cy="40" r="34" fill="none" stroke="currentColor" strokeWidth="6" strokeDasharray={213.6} strokeDashoffset={213.6 * (1 - progressPercent / 100)} strokeLinecap="round" className="text-health-green transition-all duration-1000 ease-out"/>
          </svg>
      ...
      <section className="bg-emerald-900 text-white rounded-[2.5rem] p-8 relative overflow-hidden shadow-xl shadow-emerald-900/10">
        ...
        <div className="relative z-10 space-y-6">
          ...
          {primaryTask && (<div className="bg-white/10 border border-white/10 rounded-2xl p-4 flex items-center justify-between">
              <div>
          ...
          <Button className="w-full h-14 bg-health-green hover:bg-health-green/90 text-white rounded-2xl font-bold text-sm shadow-lg shadow-emerald-950/20" onClick={() => {
      ...
      {/* AI Coach Insight */}
      {insights?.message && (<section className="bg-health-green/5 border border-health-green/10 rounded-[2.5rem] p-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center">
            ...
            <div className="flex flex-col items-end">
              ...
              <div className="w-20 h-1 bg-slate-50 rounded-full mt-1 overflow-hidden">
                <div className="h-full bg-health-green transition-all duration-500" style={{ width: `${Math.min(100, ((dailyLog?.water_glasses || 0) / 8) * 100)}%` }}/>
              </div>
            </div>
        </div>
        <div className="grid grid-cols-4 gap-3">
            ...
            return (<button key={i} onClick={() => updateWater(isActive ? i : i + 1)} className={cn("h-12 rounded-2xl flex items-center justify-center transition-all border-2", isActive ? "bg-health-green border-health-green text-white" : "bg-slate-50 border-transparent text-slate-200")}>
      ...
      {/* Progress Snapshot */}
      {(customer?.weight_kg || customer?.target_weight_kg) && (<section className="bg-slate-50 rounded-[2.5rem] p-8 border border-slate-100 grid grid-cols-3 gap-4">
          <div className="space-y-1">
      ...
      <div id="tasks-section" className="space-y-10 pt-4">
        ...
        {slotsOrder.map(slot => {
            ...
            if (!slotTasks?.length)
                return null;
            return (<div key={slot} className="space-y-4">
              ...
              <div className="space-y-3">
                {slotTasks.map((task) => {
                    const isCompleted = dailyLog?.task_completions?.some((c) => c.day_task_id === task.id) || false;
                    return (<button key={task.id} onClick={() => handleToggleTask(task.id, isCompleted)} className={cn("w-full flex items-center gap-4 p-5 rounded-[2rem] border transition-all duration-300 text-left", isCompleted ? "bg-slate-50 border-slate-100 opacity-60" : "bg-white border-slate-100 shadow-sm hover:border-health-green/20")}>
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all border-2", isCompleted ? "bg-health-green border-health-green text-white" : "bg-slate-50 border-slate-100 text-slate-200")}>
                        {isCompleted ? <Check className="w-5 h-5"/> : <div className="w-1.5 h-1.5 rounded-full bg-slate-200"/>}
                      </div>

                      <div className="flex-1 min-w-0">
                        <h4 className={cn("font-bold text-ink leading-tight text-sm", isCompleted && "line-through text-slate-400")}>
                          {task.title}
                        </h4>
                        {task.detail && (<p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                            {task.detail}
                          </p>)}
                      </div>
                    </button>);
      ...
      <section className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-8">
        ...
        ].map(mood => (<button key={mood.label} onClick={() => updateLogFn({ data: { customerId: customer.id, logDate: data.todayStr, dayNumber, mood: mood.label } })} className={cn("flex-1 flex flex-col items-center gap-3 py-6 rounded-3xl transition-all border-2", dailyLog?.mood === mood.label ? "bg-health-green border-health-green text-white" : "bg-slate-50 border-transparent text-slate-400")}>
      ...
      <section className="pt-8">
        <div className="bg-ink rounded-[2.5rem] p-8 space-y-4">
          ...
          <Button asChild variant="outline" className="w-full h-14 border-white/20 text-white hover:bg-white/10 rounded-2xl font-bold flex gap-2">
