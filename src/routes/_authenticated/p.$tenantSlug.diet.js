...
import { Utensils, Leaf, Info, ChevronRight, BookOpen, ArrowRight, Target, Sparkles } from 'lucide-react';
...
function DietPage() {
    ...
    return (<div className="max-w-md mx-auto px-6 pt-16 pb-32 animate-in fade-in duration-700 space-y-10">
      <header>
      ...
      <div className="flex bg-slate-50 p-1.5 rounded-[2rem] gap-1 sticky top-24 z-30 shadow-sm border border-slate-100">
        <button onClick={() => setActiveTab('plan')} className={cn("flex-1 py-4 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all", activeTab === 'plan' ? "bg-white text-ink shadow-sm" : "text-slate-400 hover:text-slate-600")}>
            My Plan
        </button>
        <button onClick={() => setActiveTab('free')} className={cn("flex-1 py-4 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all", activeTab === 'free' ? "bg-white text-ink shadow-sm" : "text-slate-400 hover:text-slate-600")}>
            Free Foods
        </button>
        <button onClick={() => setActiveTab('recipes')} className={cn("flex-1 py-4 rounded-[1.8rem] text-[10px] font-black uppercase tracking-widest transition-all", activeTab === 'recipes' ? "bg-white text-ink shadow-sm" : "text-slate-400 hover:text-slate-600")}>
            Recipes
        </button>
      </div>
      ...
      {activeTab === 'free' && (<div className="space-y-10 animate-in fade-in duration-700">
          ...
          <div className="space-y-10">
              {['unlimited', '1_serving', '2_servings'].map(cat => {
                ...
                if (!items?.length)
                    return null;
                return (<div key={cat} className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 gap-4">
                              {items.map(food => (<div key={food.id} className="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm flex justify-between items-center group hover:border-health-green/20 transition-all duration-500">
      ...
      {activeTab === 'recipes' && (<div className="space-y-6 animate-in fade-in duration-700">
            {recipes.map(recipe => (<button key={recipe.title} className="w-full text-left bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm group space-y-6 transition-all duration-500 hover:shadow-md hover:border-health-green/20">
