  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 mt-2 font-medium">Athletes nearing the end of their program. Time to reach out!</p>
      </div>
      ...
      <div className="grid gap-6">
          ...
          <Card key={customer.id} className="bg-white border-slate-100 rounded-[2rem] shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
            <CardContent className="p-8 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="flex items-center gap-6 flex-1">
                <div className="w-16 h-16 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-colors">
              ...
              <div className="flex items-center gap-4 w-full md:w-auto">
                <Button 
                  className="flex-1 md:flex-none h-14 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl px-8 shadow-lg shadow-emerald-600/10 active:scale-95 transition-all"
                  onClick={() => {
                    const msg = `Hi ${customer.name}! You're on Day ${customer.day_number} of ${customer.program_name}. It's time to reorder for your next phase! Shall I send you the link?`;
                    window.open(`https://wa.me/${customer.phone.replace(/\D/g, '')}?text=${encodeURIComponent(msg)}`);
                  }}
                >
                  <MessageSquare className="mr-2 h-5 w-5" /> Message on WhatsApp
                </Button>
        ...
        {!isLoading && customers?.length === 0 && (
          <div className="h-64 flex flex-col items-center justify-center text-slate-400 bg-white border border-dashed border-slate-200 rounded-[2.5rem]">
            ...
            <p className="text-sm mt-1">No customers currently needing reorder attention.</p>
          </div>
        )}
      </div>
    </div>
