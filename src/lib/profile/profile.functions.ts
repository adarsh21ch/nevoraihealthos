...
export const resetParticipantDay = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const { supabase, userId } = context;
    const { getISTDateString } = await import("../date-utils");
    
    // 1. Get today's IST date string
    const todayStr = getISTDateString();

    // 2. Update the participant's start_date in participant_programs
    // We update the most recent program record
    const { data: activeProgram, error: fetchError } = await supabase
